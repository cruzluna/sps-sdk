...
> [!NOTE]
> Once this package is [published to npm](https://app.stainless.com/docs/guides/publish), this will become: `npm install system-prompt-storage`
...
1. Via the `SYSTEM_PROMPT_STORAGE_LOG` environment variable
...
const client = new SystemPromptStorage({
  logger: logger.child({ name: 'SystemPromptStorage' }),
...
const client = new SystemPromptStorage({ fetch });
```
...
```ts
import SystemPromptStorage from 'npm:system-prompt-storage';
