...
import { APIPromise } from 'system-prompt-storage/core/api-promise';

import util from 'node:util';
import SystemPromptStorage from 'system-prompt-storage';
import { APIUserAbortError } from 'system-prompt-storage';
...
describe('instantiate client', () => {
  ...
  describe('logging', () => {
    ...
    beforeEach(() => {
      process.env = { ...env };
      process.env['SYSTEM_PROMPT_STORAGE_LOG'] = undefined;
    });
    ...
    const forceAPIResponseForClient = async (client: SystemPromptStorage) => {
    ...
    test('debug logs when log level is debug', async () => {
      ...
      const client = new SystemPromptStorage({ logger: logger, logLevel: 'debug', apiKey: 'My API Key' });
    ...
    test('debug logs are skipped when log level is info', async () => {
      ...
      const client = new SystemPromptStorage({ logger: logger, logLevel: 'info', apiKey: 'My API Key' });
    ...
    test('debug logs happen with debug env var', async () => {
      ...
      process.env['SYSTEM_PROMPT_STORAGE_LOG'] = 'debug';
      const client = new SystemPromptStorage({ logger: logger, apiKey: 'My API Key' });
    ...
    test('warn when env var level is invalid', async () => {
      ...
      process.env['SYSTEM_PROMPT_STORAGE_LOG'] = 'not a log level';
      const client = new SystemPromptStorage({ logger: logger, apiKey: 'My API Key' });
      expect(client.logLevel).toBe('warn');
      expect(warnMock).toHaveBeenCalledWith(
        'process.env[\'SYSTEM_PROMPT_STORAGE_LOG\'] was set to "not a log level", expected one of ["off","error","warn","info","debug"]',
      );
    });
    ...
    test('client log level overrides env var', async () => {
      ...
      process.env['SYSTEM_PROMPT_STORAGE_LOG'] = 'debug';
      const client = new SystemPromptStorage({ logger: logger, logLevel: 'off', apiKey: 'My API Key' });
  ...
  describe('baseUrl', () => {
    ...
    afterEach(() => {
      process.env['SYSTEM_PROMPT_STORAGE_BASE_URL'] = undefined;
    });

    test('explicit option', () => {
      const client = new SystemPromptStorage({ baseURL: 'https://example.com', apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://example.com');
    });

    test('env variable', () => {
      process.env['SYSTEM_PROMPT_STORAGE_BASE_URL'] = 'https://example.com/from_env';
      const client = new SystemPromptStorage({ apiKey: 'My API Key' });
    ...
    test('in request options overridden by env variable', () => {
      process.env['SYSTEM_PROMPT_STORAGE_BASE_URL'] = 'http://localhost:5000/env';
      const client = new SystemPromptStorage({ apiKey: 'My API Key' });
      expect(client.buildURL('/foo', null, 'http://localhost:5000/option')).toEqual(
  ...
  test('maxRetries option is correctly set', () => {
    const client = new SystemPromptStorage({ maxRetries: 4, apiKey: 'My API Key' });
    expect(client.maxRetries).toEqual(4);

    // default
    const client2 = new SystemPromptStorage({ apiKey: 'My API Key' });
...
describe('retries', () => {
  test('retry on timeout', async () => {
    ...
    const client = new SystemPromptStorage({ apiKey: 'My API Key', timeout: 10, fetch: testFetch });
  ...
  test('retry count header', async () => {
    ...
    const client = new SystemPromptStorage({ apiKey: 'My API Key', fetch: testFetch, maxRetries: 4 });
  ...
  test('retry on 429 with retry-after', async () => {
    ...
    const client = new SystemPromptStorage({ apiKey: 'My API Key', fetch: testFetch });
